...
func newSparseCountVerifyColl(t *testing.T, fx *fixture, name string, alsoPlainIndex bool) Collection {
	t.Helper()
...
// TestIndex_UniqueSparse_CountVerifyProbeKeepsMatchingDocs is the minimal case, and it does
// not need a reopen: when the sparse index is the only single-field index on
// the field, every session gets the wrong Count.
func TestIndex_UniqueSparse_CountVerifyProbeKeepsMatchingDocs(t *testing.T) {
	fx := newFixture(t)
	coll := newSparseCountVerifyColl(t, fx, "types", false)
...
// TestIndex_UniqueSparse_CountVerifyProbeStableAcrossReopen shows the second-order effect. With
...
func TestIndex_UniqueSparse_CountVerifyProbeStableAcrossReopen(t *testing.T) {
	skipIfInMemory(t)
	...
	coll := newSparseCountVerifyColl(t, fx1, "types", true)
...
// TestIndex_UniqueSparse_CountVerifyProbeUnaffectedShapes pins the boundary of the defect, so a fix
...
func TestIndex_UniqueSparse_CountVerifyProbeUnaffectedShapes(t *testing.T) {
	fx := newFixture(t)
	coll := newSparseCountVerifyColl(t, fx, "types", false)
