...
**STATUS: IMPLEMENTED (2026-07-16, this branch).** Increment 1 = `c307d29`, Increment 2 = `cbdf9ae` (+ a CoverIter finding below), Increment 3 = `193bfd0`. Acceptance reached: `DF_STRICT=1` full storetest green, `dfKnownOpen` EMPTY, all 8 repros + 4-seed soak green, benchstat within noise on every group. One addition beyond this plan, found during implementation: the multi-bound `CoverIter` path (unique multikey index + full-key `$in`) had the same dedup-above-cutoffs shape and got the same `DocDedupIter` wrap. One refinement: the order-providing gate is direction-aware for SINGLE-FIELD indexes (a lower cut only breaks ascending/min, an upper cut only breaks descending/max), which keeps lower-bounded descending scans order-providing — two pre-existing tests pin that shape and the matching-doc argument (max ≥ bound ⇒ max in bounds) makes it sound.

**Post-review corrections (2026-07-17, multi-agent review of the branch):** (1) this doc's "compound ascending without bounds is safe" claim was WRONG for element types tagged above TypeArray (object/binary/vectorF32/objectId — the whole-array entry then PRECEDES the elements in a forward scan), so the gate demotes ExactSort for **every compound run** over unproven data; the direction-aware refinement survives only for single-field indexes, which are immune via CanonicalKeyDedupIter. (2) `CoverIter`'s multikey probe reads the array tag at byte 0 and FALSE-NEGATIVES on compound indexes with the array in a suffix field (its old comment claimed conservative-true — inverted); compound multi-bound lookups now tag `multiKey=true` unconditionally. (3) `CountEntries` Branch 1 dropped the standalone single-field disjunct (`FullKeyBound ∨ ScalarProven` subsumes the sound cases; a single-field RANGE bound was latently unsound). Plus cleanups: shared `fullKeyPointBound()` predicate, Count-path proof Get narrowed to the consuming shape, dedup wrap skipped when the proof is resolved true.
