...
Every range index records the format it was built under. When a release changes how an index derives its entries, opening the database with that release rebuilds the indexes the change affects, in one write transaction, before `Open` returns; unaffected indexes are left as they are, and the rebuild takes as long as creating those indexes would. A unique index whose data the new format finds duplicate (explicit nulls under a sparse index, elements of an array under a dotted path) is left as it was and is never used by the planner until it is dropped and recreated or the duplicates are removed.
